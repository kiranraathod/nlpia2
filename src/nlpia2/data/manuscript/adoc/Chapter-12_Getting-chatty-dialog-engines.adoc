...
Mental health chatbots ride on a different quality of chatbots - our tendency to personify them. 
Therapy chatbots, such as Woebot footnote:[https://woebot.io/] and Wysa,footnote:[wysa.io/] sound enough like a human therapist to make the user feel comfortable sharing their feelings - but also (relatively) private and confidential, allowing them to overcome the awkwardness of talking about sentive, emotional issues with another human. 
...
This chatbots are built for people in underserved communities, from new immigrants in the United States to teens in the Global South. 
...
However, the way these NLP techniques fit together into a chatbot architecture can vary greatly. 
Let's look at some of the common chatbot architectures used nowadays.
...
=== Codifying chatbot states into a database

As the state structure of the bot becomes more and more complex, you would want to organize them in a format that will facilitate faster retrieval and easier navigation along the chatbot tree. 
...
We will now discuss two popular open-source Python chatbot frameworks for building chatbots with configurable NLP capabilities: nudger and RASA. 

=== Building a chatbot with nudger
...
The basic units of a conversation in RASA are a user intent and a bot action - which can be as simple as a pre-programmed utterance or a complex action programmed in Python that results in interaction with other systems - such as saving or retrieving data from database, or invoking a Web API.
...
Here is a simple set of configuration RASA files that together create a simple bot: 
...
Before you'll be able to answer that, you need to smart about keeping all of your chatbot's data in one place so that it can be easily analyzed.
...
// FIXME: complete this thought and discuss the database approach in more detail.There are a lot of metrics that you can measure when tracking your chatbot, and they usually belong to one of the three buckets: 
...
As chatbot can be considered a type of web-based (or mobile-based) user interface, a lot of the metrics used to measure web apps apply to chatbots as well. 
...
But just as human happiness, user happiness is not easily defined and measured.
In most cases, to know how the user feels about the bot, we will proactively ask them about their experience. 
Some common measures of happiness include the Net Promoter Score (NPS), which is calculated using a simple question: "Would you recommend this chatbot to your friend or colleague?"

Finally, _retention_ addresses the question of how many users come back to your chatbot after their first interaction.
It's common to measure retention over time, such daily, weekly and monthly retention. 
