...
You can use `dotenv.load_dotenv()` on your .env or you can set the variable directly using `os.environ`, like you see here:
...
We'll need to tweak our prompt to make sure that doesn't happen. 
...
First, let's update our prompt a little bit to make the bot recreate the onboarding conversation we implemented before.
...
All it does is to format the conversation history into a string and store it in a variable that you can use in your template.
...
As your chatbot gets more sophisticated, you can try other types of memory, such as `ConversationKGMemory` that turns the conversation history into a knowledge graph. 
Another useful type of memory is `ConversationSummaryMemory`, that uses another LLM to summarize the message history. 
...
The student was incorrect by completing the sequence "2,4,6,8" and answering with "12".
...
Rarely, Large Language Models can be trusted to lead a short conversation with the user - and in this case, the are heavily tested and the LLM's responses are carefully curated. 

== Maintaining your chatbot's design

Now that you've learned a few approaches to build your chatbot, it's time to address the next stage. 
...
With a tutor chatbot such as Rori, we are not only interested in the total number of users, but also in users that make it through the onboarding to the math section, and in users that have returned for a second session.
...
Impact metrics measure quantitatively the answer to the key questions - what is your chatbot's impact on the user?
Does it help your users - and your team - to reach their goals?
