...
And each fact you extract will create a new connection in a data structure called a _knowledge graph_ or _knowledge base_.
...
You will need to store the facts you've extracted in a a _knowledge base_.
A knowledge base is a database that stores knowledge in an efficient data structure called a _knowledge graph_.
This allows you to ask questions about the logical relationship between things.
A knowledge base has a query language similar to SQL which you can use to ask questions about the information stored in the knowledge base. 
And you can use a knowledge base to programmatically generate text that makes sense because it is grounded in facts in your database.
...
Once you have a knowledge graph, your chatbots and AI agents will have a way to correctly reason about the world in an explainable way that agrees with the knowledge you've given it.
...
// FIXME: load wikipedia page and text
...
The SpaCy part of speech (POS) tagger will tag the tokens in a sentence with their part of speech and their role in the dependency tree.
You can see each word's children build the tree structure you saw in Chapter 2.
...
>>> FIXME: see timnit-gebru-spacy-ents.hist.py
----
