...
And there's another advantage of re
Instead of waiting for each neuron or layer in an RNN to produce it's output before  
recursive function is just intended to be run recursively
...
_Scalability_:: Training large-capacity transformers on web scale data gives transformers unexpected (emergent) capabilities 

////
TODO HL: Use this at the end of the chapter as an alternative wording?
* Global self-attention - word patterns can be recognized no matter how far apart they are  
...
The attention mechanism removes recurrence feature of the encoder and decoder networks.
...
FIXME: Trans 
Transformers are able to generate longer and longer texts as you add layers.

As with CNNs, RNNs, and LSTMs each layer's encoding gives you a representation of the meaning of the text in a hierarchy of meaning and concepts.
...
And transformer layers are designed to have inputs and outputs with the same size and shape, so that the transformer layers can be stacked like Lego bricks that all have the same shape.
