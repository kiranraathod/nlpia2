...
...     })
----
<1> You only need to specify the first 5 or 6 characters of the git commit hash for the model you want to load

And that's it! Now, instead of HuggingFace, your model would query Replicate's API.
First, let's set your prompt: 

[source, python]
----
>>> prompt = PromptTemplate(
        input_variables = ["history", "input"],
        template="""This is a conversation between math teacher and a third-grade student. The teacher asks math question of the student and evaluates the results.
                Complete the conversation one response at a time. 
...
<2> Give your user a role name such as 'student' to improve LLM NLU accuracy and improve the readability of the message log for you
...
...    template="""You are a math teacher that's teaching math 
      ...
      with the last number in the sequence:
                - 9,10,11,12
                - 38,39,40,41
                - 2,4,6,8
                - 1,5,9,13
...
//TODO if we have time - history example.
// You can see that even with grounding, our model can't evaluate the students' answers consistently. 
// Maybe it can do better in some other domain?
// Let's try history.
