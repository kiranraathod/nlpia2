...
from .constants import DATA_DIR, BIGDATA_DIR
from .constants import DATA_INFO_FILE, BIGDATA_INFO_FILE, BIGDATA_INFO_LATEST
from .constants import INT_MIN, INT_NAN, MIN_DATA_FILE_SIZE
from .constants import EOL  # noqa (not used)
from .constants import tqdm, no_tqdm
from .constants import LINESEP

from .futil import mkdir_p, path_status, find_files  # from pugnlp.futil
from .futil import find_filepath, expand_filepath, normalize_filepath, normalize_ext, ensure_open
from .futil import read_json, read_text, read_csv
from .web import get_url_filemeta
from .web import dropbox_basename, get_url_title, try_parse_url  # noqa (not used)
from .web import requests_get
...
ZIP_PATHS = [[Path(BIGDATA_DIR) /  fn for fn in ZIP_FILES[k]] if ZIP_FILES[k] else k for k in ZIP_FILES.keys()]
...
def load_imdb_df(dirpath=BIGDATA_DIR / 'aclImdb', subdirectories=(('train', 'test'), ('pos', 'neg', 'unsup'))):
    ...
    for subdirs in tqdm(list(product(*subdirectories))):
        urlspath = dirpath / subdirs[0] / 'urls_{}.txt'.format(subdirs[1])
        if not urlspath.is_file():
        ...
        textsdir = Path(dirpath, subdirs[0], subdirs[1])
        if not Path(textsdir).is_dir():
            log.warning('Unable to find expected IMDB review text subdirectory: {}'.format(textsdir))
            continue
        filenames = [p.name for p in Path(textsdir).glob('*.[tT][xX][tT]') if p.suffix.lower() == '.txt']
        ...
        texts = []
        for fn in filenames:
            with ensure_open(Path(textsdir, fn)) as f:
                texts.append(f.read())
...
def load_glove(filepath, batch_size=1000, limit=None, verbose=True):
    ...
    >>> wv = load_glove(Path(BIGDATA_DIR, 'glove_test.txt'))
...
def load_glove_df(filepath, **kwargs):
    ...
    >>> df = load_glove_df(Path(BIGDATA_DIR, 'glove_test.txt'))
    >>> df.index[:3]
...
#     # glove_input_file = Path(BIGDATA_DIR, filepath)
#     word2vec_output_file = Path(BIGDATA_DIR, filepath.split(os.path.sep)[-1][:-4] + '.w2v.txt')
#     if not isfile(word2vec_output_file):  # TODO: also check file size
...
def load_anki_df(language='deu'):
    ...
    """
    if Path(language).is_file():
        filepath = language
        lang = re.search('[a-z]{3}-eng/', filepath).group()[:3].lower()
    else:
        lang = (language or 'deu').lower()[:3]
        filepath = Path(BIGDATA_DIR, '{}-eng'.format(lang), '{}.txt'.format(lang))
    ...
    for num_dim in (50, 100, 200, 300):
        ...
        for suffixes, num_words in zip(
            ...
            for suf in suffixes[:-1]:
                ...
                bigurls[name].append(Path(dirname, filename))
...
DATA_NAMES = {
    'pointcloud': Path(DATA_DIR, 'pointcloud.csv.gz'),
    # TBD: https://proai.org/nlpia-pointcloud.csv.gz
    'hutto_tweets0': Path(DATA_DIR, 'hutto_ICWSM_2014/tweets_GroundTruth.csv.gz'),
    # TBD: https://proai.org/tweets-2014.csv.gz
    'hutto_tweets': Path(DATA_DIR, 'hutto_ICWSM_2014/tweets_GroundTruth.csv'),
    'hutto_nyt': Path(DATA_DIR, 'hutto_ICWSM_2014/nytEditorialSnippets_GroundTruth.csv.gz'),
    # TBD: https://proai.org/nyt-editorials.csv.gz
    'hutto_movies': Path(DATA_DIR, 'hutto_ICWSM_2014/movieReviewSnippets_GroundTruth.csv.gz'),
    # https://proai.org/movie-reviews.csv.gz
    'hutto_products': Path(DATA_DIR, 'hutto_ICWSM_2014/amazonReviewSnippets_GroundTruth.csv.gz'),
...
def rename_file(source, dest):
    ...
    >>> fout = ensure_open(Path(tmpdir, 'fake_data.bin.gz'), 'w')
    >>> fout.write(b'fake nlpia.loaders.rename_file')
    30
    >>> fout.close()
    >>> dest = rename_file(Path(tmpdir, 'fake_data.bin.gz'), Path(tmpdir, 'Fake_Data.bin.gz'))
    >>> isfile(Path(tmpdir, 'Fake_Data.bin.gz'))
    True
    """
...
def normalize_ext_rename(filepath):
    ...
    >>> pth = Path(DATA_DIR, 'sms_slang_dict.txt')
...
def untar(fname, verbose=True):
    """ Uunzip and untar a tar.gz file into a subdir of the BIGDATA_DIR directory """
    if fname.lower().endswith(".tar.gz"):
        dirpath = Path(BIGDATA_DIR, Path(fname).name[:-7])
        if Path(dirpath).is_dir():
            return dirpath
        with tarfile.open(fname) as tf:
            members = tf.getmembers()
            for member in tqdm(members, total=len(members)):
                tf.extract(member, path=BIGDATA_DIR)
        dirpath = Path(BIGDATA_DIR, members[0].name)
        if Path(dirpath).is_dir():
            return dirpath
    else:
...
def get_leet_map():
    ...
    leet_path = Path(DATA_DIR, 'l33t.csv')
...
def get_netspeak_map():
    ...
    csv_path = Path(DATA_DIR, 'netspeak.csv')
...
#######################################################################
# Populate some local string variables with text files from DATA_DIR
for filename in TEXTS:
    with ensure_open(Path(DATA_DIR, filename)) as fin:
...
def unzip(filepath, verbose=True):
    ...
    unzip_dir = Path(BIGDATA_DIR, unzip_dir)
    if not unzip_dir.is_dir() or not len(unzip_dir.glob('*')) == len(z.filelist):
        z.extractall(path=unzip_dir)

    log.info('unzip_dir contains: {}'.format(list(unzip_dir.glob('*'))))
    # for f in listdir(unzip_dir):
    #     if f.lower().endswith('about.txt'):
    #         os.remove(Path(unzip_dir, f))
    for f in tqdm_prog(list(unzip_dir.glob('*'))):
        if f[-1] in ' \t\r\n\f':
            bad_path = Path(unzip_dir, f)
    ...
    anki_paths = [Path(unzip_dir, f) for f in listdir(unzip_dir)
                  if f.lower()[:3] in ANKI_LANGUAGES and f.lower()[3:] == '.txt']
    log.info('anki_paths: {}'.format(anki_paths))

    w2v_paths = [Path(BIGDATA_DIR, f[:-4] + '.w2v.txt') for f in listdir(unzip_dir)
                 if f.lower().endswith('.txt') and 'glove' in f.lower()]
    for f, word2vec_output_file in zip(listdir(unzip_dir), w2v_paths):
        glove_input_file = Path(unzip_dir, f)
    ...
    txt_paths = [Path(BIGDATA_DIR, f.lower()[:-4] + '.txt') for f in listdir(unzip_dir) if f.lower().endswith('.asc')]
    for f, txt_file in zip(listdir(unzip_dir), txt_paths):
        if f.lower().endswith('.asc'):
            input_file = Path(unzip_dir, f)
...
def download_unzip(names=None, normalize_filenames=False, verbose=True):
    r""" Download CSV or HTML tables listed in `names`, unzip and to DATA_DIR/`names`.csv .txt etc
    ...
    for name in names:
        ...
        if name in BIG_URLS:
            ...
            file_paths[name] = Path(DATA_DIR, name + '.csv')
...
def download_file(url, data_path=BIGDATA_DIR, filename=None, size=None, chunk_size=4096, normalize_filename=False, verbose=True):
    ...
    >>> pathend = Path(*('nlpia/src/nlpia/bigdata/ubuntu_dialog_test.csv.gz'.split('/')))
...
def read_named_csv(name, data_path=DATA_DIR, nrows=None, verbose=True):
    ...
    if isfile(Path(data_path, name)):
        return read_csv(Path(data_path, name), nrows=nrows)
    if name in DATASET_NAME2FILENAME:
        ...
        if filename.lower().endswith('.txt') or filename.lower().endswith('.txt.gz'):
            return read_text(Path(data_path, filename), nrows=nrows)
        elif filename.lower().endswith('.bin.gz'):
            ipdb.set_trace()
            return KeyedVectors.load_word2vec_format(Path(BIGDATA_DIR, name + '.bin.gz'), binary=True)
    try:
        return read_csv(Path(data_path, name + '.csv.gz'), nrows=nrows)
    except IOError:
        pass
    try:
        return read_csv(Path(data_path, name + '.csv'), nrows=nrows)
    except IOError:
        pass
    try:
        return read_json(Path(data_path, name + '.json'))
    except IOError:
        pass
    try:
        return read_text(Path(data_path, name + '.txt'), verbose=verbose)
    except IOError:
        pass

    # FIXME: mapping from short name to uncompressed filename
    # BIGDATA files are usually not loadable into dataframes
    filepath = Path(BIGDATA_DIR, name + '.bin.gz')
    if isfile(filepath):
        try:
            ipdb.set_trace()
            ...
            pass
    filepath = Path(BIGDATA_DIR, name + '.txt')
    if isfile(filepath):
...
def get_data(name='sms-spam', nrows=None, limit=None):
    ...
    if name in BIG_URLS:
        ...
        return read_named_csv(name, nrows=nrows)
    elif isfile(Path(DATA_DIR, name)):
        return read_named_csv(Path(DATA_DIR, name), nrows=nrows)

    msg = 'Unable to find dataset "{}"" in {} or {} (*.csv.gz, *.csv, *.json, *.zip, or *.txt)\n'.format(
        name, DATA_DIR, BIGDATA_DIR)
...
DATASET_FILENAMES = [f['name'] for f in find_files(DATA_DIR, ext='.csv.gz', level=0)]
DATASET_FILENAMES += [f['name'] for f in find_files(DATA_DIR, ext='.csv', level=0)]
DATASET_FILENAMES += [f['name'] for f in find_files(DATA_DIR, ext='.json', level=0)]
DATASET_FILENAMES += [f['name'] for f in find_files(DATA_DIR, ext='.txt', level=0)]
DATASET_NAMES = [
    f[:-4] if f.endswith('.csv') else f for f in [Path(f).suffix.lower() for f in DATASET_FILENAMES]]
...
def clean_cornell_movies(filename='cornell_movie_dialogs_corpus.zip', subdir='cornell movie-dialogs corpus'):
    ...
    if fullpath_zipfile.lower().endswith('.zip'):
        ...
        dirname = dirname[:-4]
    fullpath_movie_lines = Path(BIGDATA_DIR, dirname, subdir, 'movie_lines.txt')
    dialog = pd.read_csv(
...
def isglove(filepath):
    ...
    >>> isglove(Path(DATA_DIR, 'cats_and_dogs.txt'))
    False
    """
...
def nlp(texts, lang='en', linesep=None, verbose=True):
    ...
    linesep = LINESEP if LINESEP in ('default', True, 1, 'os') else LINESEP
...
def clean_win_tsv(filepath=Path(DATA_DIR, 'Products.txt'),
